...
recommandation_sante(Sommeil, HeartRate, IMC, Tabac, Alcool, Alimentation) :-
  number(Sommeil),
  number(HeartRate),
  number(IMC),
  HeartRate >= 60,
  format('Recommendation sante checked.~n', []).
...
get_recommendations(Age, Sexe, Poids, Taille, Sommeil, Activite, Activite_intense,Musculation, Kcal, Eau, Tabac, Alcool, Alimentation,Freq_cardiaque, VitamineD_consomme, TempsExposition,SurfaceExposee, Recommendations) :- 
    % Calcul de l IMC
    calcul_imc(Poids, Taille, IMC),
    ...
    % Résultat global sous forme de liste
    Recommendations = [
        [state: "sommeil", recommandation: MessageSommeil],
        [state: "activites", recommandation: MessageActivite],
        [state: "calories", recommandation: MessageKcal],
        [state: "eau", recommandation: MessageEau],
        [state: "stress", recommandation: MessageStress],
        [state: "vitamine_d", recommandation: MessageVitamineD]
    ].

    format('Recommendations computed.~n', []).

% Example usage:
% ?- get_recommendations(25, homme, 150, 1.70, 8, 120, 50, 2, 2800, 3, non, non, equilibree, 80, 16, 4, grande, Recommendations).
% Recommendations = [{state:"sommeil", recommandation:"Votre temps de sommeil respecte les recommandations pour votre âge. Continuez ainsi pour une bonne santé !"}, ...].
